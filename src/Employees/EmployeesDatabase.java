...
public class EmployeesDatabase {
  public static void AddEmployee(Employee e, Database database) throws SQLException {
      String insert = "INSERT INTO `employees`(`ID`, `Name`, `Email`, `Tel`, `Salary`, `Position`) " +
              "VALUES ('" + e.getId() + "', '" + e.getName() + "', '" + e.getEmail() + "', '" +
...
public static void DeleteEmployee(String id,Database database) throws SQLException {
      ...
      database.getStatement().execute(delete);
        ...
        String update= "UPDATE `employees` SET `Name`='" + e.getName() + "', `Email`='" + e.getEmail() + "', `Tel`='" + e.getTel() + "', `Salary`='" + e.getSalary() + "', `Position`='" + e.getPosition() + "' WHERE `ID`=" + e.getId() + ";";
        database.getStatement().execute(update);
    }

    public static  String[] getEmployeeNames(Database database) throws SQLException {
      ArrayList<Employee> employees=getAllEmployees(database);
